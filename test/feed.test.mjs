...
const rss=(title,link='https://example.org/news',date=new Date().toUTCString(),extra='',sourceUrl='https://example.org')=>`<?xml version="1.0"?><rss version="2.0"><channel><title>Feed</title><link>https://example.org</link><item><title>${title}</title><link>${link}</link><pubDate>${date}</pubDate><source url="${sourceUrl}">Fonte</source>${extra}</item></channel></rss>`;
...
test('RSS trata CDATA, entidades e um único item; não usa descrição Google como resumo',()=>{const [n]=parseFeed(rss('<![CDATA[Recolha & compostagem - Fonte]]>','https://example.org/a?x=1&amp;y=2',new Date().toUTCString(),'<description>&lt;a href="x"&gt;Recolha&lt;/a&gt;</description>'),def,topic);assert.equal(n.title,'Recolha & compostagem');assert.equal(n.description,'');assert.equal(n.url,'https://example.org/a?x=1&y=2');assert(n.topics.includes('biorresiduos'));});
test('Rejeita URLs executáveis, datas inválidas, antigas e futuras',()=>{assert.equal(safeURL('javascript:alert(1)'),'');assert.equal(safeURL('https://user:pass@example.org'),'');for(const [url,date] of [['javascript:alert(1)',new Date().toUTCString()],['https://example.org','invalid'],['https://example.org','Sat, 01 Jan 2000 00:00:00 GMT'],['https://example.org',new Date(Date.now()+86400000).toUTCString()]])assert.equal(parseFeed(rss('Teste',url,date),def,topic).length,0);});
test('A pesquisa unificada mantém os artigos limitados aos últimos 30 dias',()=>{const recent=rss('Projeto de compostagem','https://example.org/recent',new Date(Date.now()-20*86400000).toUTCString()),old=rss('Projeto de compostagem','https://example.org/old',new Date(Date.now()-100*86400000).toUTCString());assert.equal(parseFeed(recent,def,topic).length,1);assert.equal(parseFeed(old,def,topic).length,0);});
test('RSS direto só entra no tema correspondente',()=>{assert.equal(parseFeed(rss('Ações do mercado tecnológico sobem'),{...def,kind:'direct'},topic).length,0);});
test('Categorias filtram resultados genéricos, vagas e portais de emprego',()=>{
 const landscape=TOPICS.find(t=>t.id==='arquitetura-paisagistica');
 assert.equal(parseFeed(rss('Empresa procura arquiteto paisagista — candidate-se já'),def,landscape).length,0);
 assert.equal(parseFeed(rss('Engenheiro hidráulico para projeto de saneamento','https://www.net-empregos.com/emprego/engenheiro-hidraulico'),def,TOPICS.find(t=>t.id==='engenharia')).length,0);
 assert.equal(parseFeed(rss('Arquiteto paisagista para projeto de parques','https://emprego.sapo.pt/oferta/123'),def,landscape).length,0);
 assert.equal(parseFeed(rss('Engenheiro de águas e saneamento','https://news.google.com/read/123',new Date().toUTCString(),'','https://www.net-empregos.com'),def,TOPICS.find(t=>t.id==='engenharia')).length,0);
 assert.equal(parseFeed(rss('A cidade apresenta um novo orçamento municipal'),def,TOPICS.find(t=>t.id==='urbanismo')).length,0);
 assert.equal(parseFeed(rss('Projeto de desenho da paisagem restaura corredor ecológico'),def,landscape).length,1);
});
test('Extrai miniaturas RSS e imagens do excerto, mantendo URLs seguras',()=>{
 ...
 const [a]=parseFeed(rss('Recolha de resíduos alimentares para compostagem','https://example.org/a',new Date().toUTCString(),extra),def,topic);
 assert.equal(a.imageUrl,'https://images.example.org/article.jpg');
 const unsafe='<media:thumbnail url="javascript:alert(1)"/>';
 const [b]=parseFeed(rss('Recolha de resíduos alimentares para compostagem','https://example.org/b',new Date().toUTCString(),unsafe),def,topic);
