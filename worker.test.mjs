...
test('API: recolha, cache, fallback após falha e expiração sem notícias fictícias',async()=>{
 ...
 try{
  let r=await worker.fetch(request(),{},ctx);let d=await r.json();assert.equal(r.status,200);assert.equal(d.items.length,1);assert.equal(calls,4);assert(d.sources.every(s=>s.state==='ok'));
  await worker.fetch(request(),{},ctx);assert.equal(calls,4);
