...
export function plain(value){return text(value).replace(/<script\b[^>]*>[\s\S]*?<\/script>/gi,'').replace(/<style\b[^>]*>[\s\S]*?<\/style>/gi,'').replace(/<[^>]+>/g,' ').replace(/&(?:amp|lt|gt|quot|apos|#39);/g,match=>({'&amp;':'&','&lt;':'<','&gt;':'>','&quot;':'"','&apos;':"'"}[match]||match)).replace(/\s+/g,' ').trim();}
...
export function languageOf(title,fallback){
 ...
 const ptWords=normalized.match(/\b(recolha|compostagem|residuos|biorresiduos|gestao|municipio|portugues|portuguesa|projeto|sustentabilidade|abastecimento|autarquia|espacos|arborizacao|mobilidade|agua|ambiente|residuos|reciclagem|co2|clima|energia|transporte|municipal|residuos|eficiencia|gestao|poluicao|verde|mobilidade|biodiversidade|floresta|desperdicio|circular|economia|saneamento|cidades|municipios)\b/g)||[];
...
const practicePattern=/projeto|project|iniciativa|initiative|pilot|solucao|solution|case stud|boas praticas|best practice|inovacao|innovation|implement|lanc|launch|roll.out|transform|reutiliz|reuse|co/gi;
export function classify(title,description){const content=normalize(title+' '+description);return TOPICS.filter(t=>t.match&&new RegExp(t.match).test(content)&&(!t.sector||new RegExp(t.sector).test(content))).map(t=>t.id||t.label||t.name).filter(Boolean);}
const jobAdTitle=/\b(job|jobs|vacancy|vacancies|career|careers|hiring|recruitment|recruiting|employment opportunity|apply now|we are hiring|oferta de emprego|ofertas de emprego|anuncio de emprego|anúncio de emprego|trabalhe connosco|trabalhe conosco)\b/i;
...
const jobBoardDomains=['net-empregos.com','expressoemprego.pt','empregosonline.pt','empregos.pt','careerjet.pt','indeed.com','indeed.pt','glassdoor.com','jooble.org','talent.com','monster.com','infojobs.pt','jobtoday.com','jobs.com'];
...
export function isJobAdvertisement(title,description='',url='',source='',sourceURL=''){return isJobBoard(url,source)||isJobBoard(sourceURL,source)||jobAdTitle.test(title)||jobAdCall.test(description)||jobAdCall.test(title);} 
export function parseFeed(xml,def,topic,mode,now=Date.now()){
 ...
 for(const row of rows.slice(0,100)){
  ...
  articles.push({id:url,title,description,url,source,sourceURL,date:new Date(ms).toISOString(),...language,imageUrl,country:host.endsWith('.pt')||def.id==='ambiente-magazine'?'PT':null,topics:classify(title,description),topic:topic.id});
 }
 return articles;
}
export function mergeArticles(groups){const byTitle=new Map();for(const article of groups.flat()){const key=normalize(article.title).replace(/[^a-z0-9]/g,'');const prev=byTitle.get(key);if(!prev){byTitle.set(key,article);continue;}if(prev.description.length<article.description.length){byTitle.set(key,article);}}return [...byTitle.values()];}
