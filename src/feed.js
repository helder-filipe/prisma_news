...
function imageFromRow(row,channel){
 ...
 return safeURL(htmlImage)||visit(row['media:thumbnail'])||visit(row['media:content'])||visit(row.enclosure)||visit(row['itunes:image'])||visit(row.image)||visit(channel.image);
}
...
export function isJobAdvertisement(title,description=''){return jobAdTitle.test(title)||jobAdCall.test(description)&&/\b(vaga|emprego|job|career|recruit|candidatur|application|cv|resume)\b/i.test(title+' '+description);}
export function parseFeed(xml,def,topic,mode,now=Date.now()){
 ...
 for(const row of rows.slice(0,100)){
  ...
  if(isJobAdvertisement(title,rawDescription))continue;
  ...
  const imageUrl=imageFromRow(row,channel);
