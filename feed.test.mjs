...
test('Categorias filtram resultados genéricos, vagas e portais de emprego',()=>{
 ...
 assert.equal(parseFeed(rss('Engenheiro hidráulico para projeto de saneamento','https://www.net-empregos.com/emprego/engenheiro-hidraulico'),def,TOPICS.find(t=>t.id==='engenharia'),'noticias').length,0);
 assert.equal(parseFeed(rss('Arquiteto paisagista para projeto de parques','https://emprego.sapo.pt/oferta/123'),def,landscape,'noticias').length,0);
 assert.equal(parseFeed(rss('Engenheiro de águas e saneamento','https://news.google.com/read/123',new Date().toUTCString(),'','https://www.net-empregos.com'),def,TOPICS.find(t=>t.id==='engenharia'),'noticias').length,0);
