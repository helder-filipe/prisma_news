...
const labelFor=id=>$('#language')?.value==='en'?(labelsEn[id]||labels[id]||'Environment'):(labels[id]||'Ambiente');
...
function tag(n){const topic=selected==='tudo'?n.topics[0]:selected;return `<div class="tag">${escapeHTML(labelFor(topic))} <span class="language" title="Língua ${n.languageMethod==='title'?'identificada no título':'estimada a partir do feed'}">${escapeHTML(n.lang.toUpperCase())}${n.languageMethod==='feed'?'*':''}</span></div>`;}
...
function render(){
 ...
 $('#heading').innerHTML=escapeHTML(selected==='tudo'?'Em perspetiva':labelFor(selected))+'<span>.</span>';
 ...
 const items=matches(data?.items||[]);$('#count').textContent=`${items.length} ${items.length===1?'artigo':'artigos'}${$('#language').value==='pt'?' · Português':$('#language').value==='en'?' · Inglês':''}`;
...
function renderTopics(){ $('#topics').innerHTML=TOPICS.map(t=>`<button type="button" data-topic="${t.id}" aria-pressed="${t.id===selected}">${escapeHTML(labelFor(t.id))}</button>`).join(''); }
renderTopics();
...
$('#search').addEventListener('input',render);$('#sort').addEventListener('change',render);$('#language').addEventListener('change',()=>{renderTopics();render();});$('#refresh').addEventListener('click',()=>load(true));
