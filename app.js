...
function matches(items){const q=normalize($('#search').value.trim());const lang=$('#language').value;return items.filter(n=>(lang==='all'||n.lang===lang)&&normalize([n.title,n.description,n.source,...n.topics.map(t=>labels[t])].join(' ')).includes(q)).sort((a,b)=>$('#sort').value==='old'?a.date.localeCompare(b.date):b.date.localeCompare(a.date));}
...
function render(){
 ...
 const items=matches(data?.items||[]);$('#count').textContent=`${items.length} ${items.length===1?'artigo':'artigos'}${$('#language').value==='pt'?' · Português':$('#language').value==='en'?' · English':''}`;
