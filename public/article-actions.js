...
export function printArticleHTML(article){const url=publicArticleURL(article);const date=new Intl.DateTimeFormat('pt-PT',{dateStyle:'long'}).format(new Date(article.date));return `<div class="print-brand">prisma◆ <span>Ficha de notícia</span></div><h1 lang="${escapeHTML(article.lang)}">${escapeHTML(article.title)}</h1><p class="print-meta">${escapeHTML(article.source)} · ${escapeHTML(date)}</p>${article.description?`<p lang="${escapeHTML(article.lang)}" class="print-excerpt">${escapeHTML(article.description)}</p>`:'<p>O feed desta publicação não disponibiliza um excerto. O texto integral pode ser consultado na fonte.</p>'}<h2>Consultar o artigo original</h2><a class="print-url" href="${escapeHTML(url)}">${escapeHTML(url)}</a><p class="print-note">Esta ficha contém a informação disponibilizada no PRISMA, não uma reprodução integral do artigo. Conteúdo e autoria pertencem à publicação de origem.</p>`;}
export function installArticleActions(getArticle){
 ...
 dialog.addEventListener('click',async e=>{
  ...
  if(e.target.closest('[data-print]')){printRoot.innerHTML=printArticleHTML(active);previousTitle=document.title;document.title='PRISMA — '+active.title;document.body.classList.add('printing-article');dialog.close();try{window.print();}catch{cleanup();}returnFocus?.focus();}
 });
